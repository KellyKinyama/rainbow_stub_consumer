...
class _FakeSession implements RtcSession {
  _FakeSession() : state = CallState.dialing {
...
class _FakeSignaling implements SfuSignaling {
  ...
  final trickles =
      <({String candidate, String? sdpMid, int? sdpMLineIndex, int target})>[];
  bool closed = false;
...
void main() {
  test('SfuGroupCallSession.connect joins the SFU with the local offer and '
      'applies the returned answer', () async {
  ...
  test('SFU-initiated offer triggers answer + sendAnswer', () async {
    final signaling = _FakeSignaling();
    final adapter = _FakeAdapter();
    final session = SfuGroupCallSession(
      signaling: signaling,
  ...
  test('SFU trickle is applied as a remote ICE candidate', () async {
    ...
    expect(
      adapter.sessions.single.remoteCandidates.single.candidate,
      contains('172.16.0.5'),
    );
    await session.close();
  });
  ...
  test(
    'GroupCallManager.startGroupCall announces marker and joins SFU',
    () async {
  ...
  test(
    'manager without sfuUrl is disabled and rejects startGroupCall',
    () async {
      final xmpp = _FakeXmpp();
      final adapter = _FakeAdapter();
      final manager = GroupCallManager(
  ...
  test(
    'leaveGroupCall closes session; announceEnd sends ended marker',
    () async {
      final xmpp = _FakeXmpp();
      final adapter = _FakeAdapter();
      final signaling = _FakeSignaling();
      ...
      expect(xmpp.markers.last, (
        room: 'r@muc.localhost',
        state: 'ended',
        sid: 'gc-2',
      ));
      await manager.dispose();
