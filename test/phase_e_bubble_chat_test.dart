...
void main() {
  ...
  test('MUC chatControllerCapsule fires a one-shot MAM query for the room JID', () async {
  ...
  test('incoming groupchat uses the resource part (nick) as authorId', () async {
  ...
  test('local group send-echo attributes authorship to the signed-in user', () async {
    final controller = container.read(chatControllerCapsule(roomJid));
  ...
  test('MAM MUC hydration inserts oldest-first with the correct nick author', () async {
    final controller = container.read(chatControllerCapsule(roomJid));
  ...
  test('resetMessagesCapsuleCache bumps generation so old capsules go dormant', () async {
